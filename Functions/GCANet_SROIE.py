    ...
    def forward(self, x, labels):
        ...
        if labels is not None:
            if len(labels.shape) == 2:
                labels_expanded = labels.unsqueeze(2).unsqueeze(3).expand(-1, -1, y1.size(2), y1.size(3))
                y1 = torch.cat([y1, labels_expanded], dim=1)
            else:
                print("Unexpected number of dimensions for labels:", len(labels.shape))
        
        gates = self.gate(torch.cat((y1, y1, y1), dim=1))
        gated_y = y1 * gates[:, [0], :, :] + y1 * gates[:, [1], :, :] + y1 * gates[:, [2], :, :]
